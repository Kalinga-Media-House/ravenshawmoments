...
test_markdown = []
for i, item in enumerate(test_categories, 1):
    name = item[0]
    payload = item[1]
    expected = item[2]
    final_state = item[3] if len(item) > 3 else "N/A"
    payload_str = payload.replace('\\n', ' ')
    test_markdown.append(f"""
### Test {i}: {name}
- **Unique descriptive name**: {name}
...
- **Exact executable SQL/RPC call**: `SELECT public.save_competition_workflow('comp-456', '{payload_str}'::jsonb);`
- **Exact JSON payload**: `{payload_str}`
...
- Scheduled -> Scheduled: `published_at=NULL, scheduled=Date`
- Scheduled -> Published: `published_at=now, scheduled=NULL`
- Published -> Draft: `published_at=NULL, scheduled=NULL`
- Published -> Scheduled: `published_at=orig, scheduled=Date`
...
- `registration_fee` numeric check using `validate_finite_numeric`.
...
out_path = r'C:\Users\biswa\.gemini\antigravity\brain\928c6dc7-e755-4c1b-844d-0ce32f328766\implementation_plan_final.md'
with open(out_path, 'w', encoding='utf-8') as f:
    f.write(markdown_content)

print("Markdown generated successfully at", out_path)
