...
export const OrganizationDirectoryCard = ({ organization }: OrganizationDirectoryCardProps) => {
  return (
    <Link 
      ...
      <div className="p-6 sm:p-7 flex flex-col h-full mt-1">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center justify-center w-11 h-11 rounded-full bg-[var(--color-heritage-gold)]/10 border border-[var(--color-heritage-gold)]/20 shrink-0 group-hover:bg-[var(--color-heritage-gold)]/20 transition-colors duration-300">
            {organization.icon && React.cloneElement(organization.icon as React.ReactElement<any>, { className: 'w-5 h-5 heritage-icon' })}
          </div>
          {organization.shortName && (
             <span className="text-[0.75rem] font-bold tracking-widest text-[var(--color-heritage-gold)] uppercase bg-[var(--color-heritage-gold)]/10 px-3 py-1 rounded-full">
               {organization.shortName}
             </span>
          )}
        </div>
        ...
        {/* Title */}
        <h3 className="text-xl sm:text-2xl font-bold heritage-card-title mb-2 leading-tight tracking-tight group-hover:text-[var(--color-heritage-gold)] transition-colors duration-300 line-clamp-2">
          {organization.name}
        </h3>
        
        {/* Category Label */}
        <div className="mb-4">
          <span className="text-[0.7rem] font-bold tracking-wider heritage-card-muted uppercase">
